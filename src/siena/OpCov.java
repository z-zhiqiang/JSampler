...
public class OpCov {
    ...
    public static boolean app_operator(short op, 
					 AttributeValue x, AttributeValue y) {
	//
	// semantics of operators in filters
	//
	// app_operator(op, x, y)  <==>  x op y
	//
	...
	case AttributeConstraint.SF: return af1.op == AttributeConstraint.SS
		     && app_operator(AttributeConstraint.SS,
				       af2.value , af1.value);
	case AttributeConstraint.PF: 
	    switch(af1.op) {
	    case AttributeConstraint.SS: 
		return app_operator(AttributeConstraint.SS,
				      af2.value,af1.value);
	    case AttributeConstraint.GT: 
		return app_operator(AttributeConstraint.LE,
				      af1.value, af2.value)
		    && !app_operator(AttributeConstraint.PF,
				       af1.value, af2.value);
	    case AttributeConstraint.LT: 
		return app_operator(AttributeConstraint.GE,
				      af1.value, af2.value)
		    && !app_operator(AttributeConstraint.PF,
				       af1.value, af2.value);
	    case AttributeConstraint.GE: 
		return app_operator(AttributeConstraint.LT,
				      af1.value, af2.value);
	    case AttributeConstraint.LE: 
		return app_operator(AttributeConstraint.GT,
				      af1.value, af2.value);
    ...
    public static boolean app(AttributeConstraint ac, AttributeValue av) {
	return app_operator(ac.op, av, ac.value);
    }

    public void add_con(Filter f) {
	    ...
	    f.addConstraint(name, ac);
    }

    //
    // semantics of subscriptions
    //
    public static boolean app(Filter f, Event e) {
...
//            AttributeConstraint ac = (AttributeConstraint)li.next();
            ...
            e.putAttribute(name,ea);	

	    while(li.hasNext() )
		if (!app((AttributeConstraint)li.next(), ea))
		    return false;
